...
for i in range(1,11):
    ip=[]
    url="https://www.bikewale.com/honda-bikes/sp-125/reviews/"+str(i)
    response=requests.get(url)
    soup=bs(response.content,"html.parser")
    reviews=soup.find_all("div",attrs={"class","collapsible-content toggle-off main-content-off font14 text-slate-grey"})
    for i in range(len(reviews)):
        ip.append(reviews[i].text)
    bikewale_reviews=bikewale_reviews+ip
    
with open("Honda-SP-125.text","w",encoding="utf8") as output:
    output.write(str(bikewale_reviews))
    
ip_rev_string=" ".join(bikewale_reviews)
...
stop_words.extend(["honda","bike","super","amazing","bestbody","design","day"])
